...
10. A driver may also configure a separate dedicated mailbox (secondary mailbox) for doing the indirect access such as for Tx Timestamping, adjusting the Device PHC etc. The Device control indicates to the driver to use a dedicated secondary mailbox or not through the PTP capability negotiation data structure by setting the mbx_q_index field. If CP supports allocation of PF/VF secondary MBXs dedicated for PTP, mbx_q_index will hold a non-zero value. If mbx_q_index is non-zero, driver should allocate secondary MBX queue for PTP. Driver should send ADD_QUEUES command and pass PTP mbx_q_index as mbx_q_index and qtype VIRTCHNL2_QUEUE_TYPE_MBX_TX/RX to inform Control Daemon to allocate new PF/VF MBX queue meant for PTP handling.  After the initial capability exchange over the default mailbox, if the Device Control suggests using a secondary mailbox, the driver sets up a secondary mailbox using the default mailbox and then switches to use secondary mailbox for any PTP related operations.
